...
const ClientExperience: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 space-y-16">
          ...
          <div className="aspect-[3/4] bg-white rounded-[2.5rem] border-2 border-dashed border-zinc-200 flex flex-col items-center justify-center relative overflow-hidden group">
            {userPhoto ? (
              <>
                ...
                <button onClick={() => setUserPhoto(null)} className="absolute top-4 right-4 bg-black/50 p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          <div className="aspect-[3/4] bg-[#fdd49e]/5 rounded-[2.5rem] border-2 border-dashed border-[#fdd49e]/30 flex flex-col items-center justify-center relative overflow-hidden group">
            {jewelryRef ? (
              <>
                ...
                <button onClick={() => setJewelryRef(null)} className="absolute top-4 right-4 bg-black/50 p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeWidth="2"/></svg>
                </button>
              </>
            ) : (
      ...
      <CameraModal isOpen={isCameraOpen} onClose={() => setIsCameraOpen(false)} onCapture={(img) => setUserPhoto(img)} mode="photo" />
    </div>
  );
};
